...
user function A250ITOK()
	...
	If SC2->(dbSeek(xFilial("SC2")+substr(M->D3_OP,1,11)))
		//Diego - 18/02/2020 Validação para não ter o apontamento maior que a ordem de produação pelos operadores
	...
	Endif
	*/
	//Verifico se o usuário definiu que haverá ajuste automático dos empenhos
		...
		if !Empty(M->D3_OP) .AND. !Empty(M->D3_COD) .AND. M->D3_PERDA > 0
			...
			while !(_cAliasSD4)->(EOF()).AND. (_cAliasSD4)->D4_FILIAL == xFilial("SD4") .AND. (_cAliasSD4)->D4_OP==M->D3_OP 
				...
				if SD4->(MsSeek(xFilial("SD4")+(_cAliasSD4)->D4_COD+(_cAliasSD4)->D4_OP+(_cAliasSD4)->D4_TRT+(_cAliasSD4)->D4_LOTECTL,.T.,.F.))
					...
					//Verifico se o tipo do produto é embalagem
						...
						if SC2->(MsSeek(xFilial("SC2")+M->D3_OP,.T.,.F.))
							...
							*/
							...
							if AllTrim(Upper(SD4->D4_AUTOAJU))<>"S"
								if _nSldEmpe == 0
									//TRECHO COMENTADO DEVIDO A MUDANÇA NO TRATAMENTO DO ZERAEMP, ATÉ A RELEASE 12.1.33 zerava somente o produto posicionado, 
									//a partir da migração zera todos os empenhos
