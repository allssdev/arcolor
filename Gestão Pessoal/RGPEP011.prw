...
#include 'protheus.ch'
#include 'parmtype.ch'
/*/{Protheus.doc} RGPEP011
@description Função de usuário para recalculo das horas extras utilizando, para funcionarios mensalistas, fator 220 horas 
...
@history 18/09/2020, Rodrigo Telecio (rodrigo.telecio@allss.com.br), Aplicação no ambiente de produção
@see https://allss.com.br
/*/
user function RGPEP011()
local nSalHora  := 0
local nDiasC	:= SuperGetMV("MV_XDIASHE",.F.,30) 
local cTab 		:= ""
local cPd  		:= ""
local cPerc		:= 100
//local cRotina	:= "RGPEP011"
local nPerc		:= 0
local aPdTmp	:= {}
local nX		:= 0
local nSalInc	:= 0
...
if SRA->RA_TIPOPGT == "M"
	if SRA->RA_CATFUNC $ "M"
		...
		BeginSql Alias cTab
			...
			WHERE RV_FILIAL  = %xFilial:SRV% 
				AND RV_TIPOCOD = '1'
				AND RV_PERC    > %Exp:cPerc%
				AND RV_HE      = 'S'
				AND SRV.%NotDel% 
			ORDER BY 
				RV_COD
		EndSql
		//MemoWrite(GetTempPath()+cRotina+"_QRY_002.txt",GetLastQuery()[02])
		dbSelectArea(cTab)
		while !(cTab)->(EOF())
			if !empty(cPd)
				cPd += ","
