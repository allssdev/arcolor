#include "totvs.ch"
/*/{Protheus.doc} RFATR035
@description Relatorio de totais FATURADOS por produto por periodo, mes a mes, baseado nos Documentos de Saída. O relatório apresentará e somará quantidades, valores de mercadorias ou valor net de vendas, conforme parametrização do usuário. Também conforme parametrização, o relatório apresentará Somente Vendas, Somente o que não for Venda ou Ambos. Além disso, também conforme parametrização, o relatório abate ou não as devoluções de vendas.
...
user function RFATR035()
	private oReport
	private oSection
	private _cRotina  := "RFATR035"
	private cPerg     := _cRotina
	private _cSD2TMP  := GetNextAlias()
	private _cFormTit := "" //_cFormTit := '"Relatório de Totais FATURADOS por Produto, por Período ("+IIF(MV_PAR33==2,"SOMENTE",IIF(MV_PAR32==1,"SEM","COM"))+" DEVOLUÇÕES) - em "+SuperGetMV("MV_MOEDAP"+MV_PAR34,,"Reais")'
	private aRegs     := {}
	private _aCpos    := {}
	private _lMoeda   := .F.
	if FindFunction("TRepInUse") .And. TRepInUse()
		...
		_cFormTit := '"Relatório de Totais FATURADOS, por Período ("+IIF(MV_PAR33==2,"SOMENTE",IIF(MV_PAR32==1,"SEM","COM"))+" DEVOLUÇÕES) - em "+SuperGetMV("MV_MOEDAP"+MV_PAR34,,"Reais")'
		// MV_PAR32 := IIF(ValType(MV_PAR32)=="N",cValToChar(MV_PAR32),MV_PAR32)
		// MV_PAR33 := IIF(ValType(MV_PAR33)=="N",cValToChar(MV_PAR33),MV_PAR33)
		if empty(MV_PAR34)
			MV_PAR34 := "1"
		endif
		...
		while MsgYesNo("Deseja emitir o relatório novamente?",_cRotina+"_001")
			if !&(_bPERG)
				return
			endif
			_cFormTit := '"Relatório de Totais FATURADOS, por Período ("+IIF(MV_PAR33==2,"SOMENTE",IIF(MV_PAR32==1,"SEM","COM"))+" DEVOLUÇÕES) - em "+SuperGetMV("MV_MOEDAP"+MV_PAR34,,"Reais")'
...
return
/*/{Protheus.doc} ReportDef (RFATR035)
...
//	oReport := TReport():New(_cRotina,cTitulo,cPerg,{|oReport| PrintRel(oReport)},"Emissao do relatório, de acordo com o intervalo informado na opção de Parâmetros.")
	...
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	oSection := TRSection():New(oReport,"RELATÓRIO DE ITENS FATURADOS - QTD. E VALOR",{"SD1","SD2","SF2","SB1","SA1","SA3","SF4"},_aOrd/*{Array com as ordens do relatório}*/,/*Campos do SX3*/,/*Campos do SIX*/)
	...
	//TRCell():New(oSection,"B1_TIPO"   ,"SB1"/*Tabela*/,RetTitle("B1_TIPO"	  ),PesqPict  ("SB1","B1_TIPO" ),TamSx3("B1_TIPO" )[1],/*lPixel*/,{|| (_cSD2TMP)->B1_TIPO   	})	// Tipo de Produto
	...
	//TRCell():New(oSection,"D2_QUANT"    ,"SUB"/*Tabela*/,RetTitle("D2_QUANT"  ),PesqPictQt("D2_QUANT"	   ),TamSx3("D2_QUANT")[1],/*lPixel*/,{|| (_cSD2TMP)->D2_QUANT      })	// Quantidade
	//TRCell():New(oSection,"D2_TOTAL"    ,"SUB"/*Tabela*/,RetTitle("D2_TOTAL"  ),PesqPict  ("SD2","D2_TOTAL"),TamSx3("D2_TOTAL")[1],/*lPixel*/,{|| (_cSD2TMP)->D2_TOTAL		})	// Valor Total
	...
	*/
return oReport
/*/{Protheus.doc} PrintRel (RFATR035)
...
//Início do tratamento para composição das devoluções vendas vinculadas as saídas filtradas
	...
	If MV_PAR32 == 3				//NF DEVOLUÇÃO 
		_qDevNF     := "("
		if MV_PAR05 == 5
			_qDevNF += " SELECT ISNULL(SUM(D1_QUANT)*SUM(D1_PESO),0) "
...
*/
		...
		ElseIf MV_PAR06 == 5
			_cFlOper := "% AND SD2.D2_TIPOPER  IN " + _cTpVC +"%"
		EndIf
	EndIf
...
//Definição do campo de valor a ser apresentado/totalizado
	if MV_PAR05 == 5 //Peso Liquido
		if SD2->(FieldPos("D2_SCOA"))<>0
			_cCpoSum := ", (CASE WHEN F4_ESTOQUE = 'S' OR (D2_TES = '601' AND D2_SCOA <> '') "
		else
			_cCpoSum := ", (CASE WHEN F4_ESTOQUE = 'S'                                       "
		endif
		_cCpoSum += "              THEN (("+IIF(MV_PAR33==2,"0","D2_QUANT*D2_PESO")+" - "+_qDevNF+")) "
		_cCpoSum += "              ELSE 0 "
		_cCpoSum += "       END)   [VALOR]"
		_cCpSuDv := ", ((D1_QUANT*D1_PESO) * (-1)) [VALOR]"
	ElseIf MV_PAR05 == 4		//Valor NET
...
//Definição das colunas de datas (Mês/Ano) NO RELATÓRIO com impacto nos totais, inclusive
	...
	Else
		...
		If len(_aCols) > 12 .AND. MV_PAR38 == 2	//Sim = Organiza os períodos por meses+ano (para comparação de mesmo mes entre anos distintos)
			...
			EndIf
        EndIf
		if MV_PAR05 == 5 //Quantidade Peso	
			TRCell():New(oSection,"TOTAL", (_cSD2TMP)/*Tabela*/,"TOTAL", "@E 999,999,999,999.99", 31, /*lPixel*/,{|| Round((_cSD2TMP)->TOTAL,TamSx3("D2_PESO"  )[02]) })// Peso Liquido
		ElseIf MV_PAR05 == 4		//Valor NET
		...
		EndIf
		if MV_PAR05 == 5		//Peso Liquido
			TRCell():New(oSection,"MEDIA", (_cSD2TMP)/*Tabela*/,"MEDIA", "@E 999,999,999,999.99", 31, /*lPixel*/,{|| Round((_cSD2TMP)->MEDIA,TamSx3("D2_PESO"  )[02]) })// Peso Liquido
		ElseIf MV_PAR05 == 4		//Valor NET
...
static function ValidPerg()
	...
	_aTam  := {01,00,"N"}
	AADD(aRegs,{cPerg,"05","Tipo de Informação?"     ,"","","mv_ch5",_aTam[03],_aTam[01]	,_aTam[02]	,1,"C","NAOVAZIO()","mv_par05","Vlr. Mercadorias" ,"","","","","Quantidade"	    ,"","","","","Vlr. Total"		,"","","","","Valor NET","","","","","Peso Liquido" ,"","","","",""      ,"",""})
	AADD(aRegs,{cPerg,"06","Operação?"               ,"","","mv_ch6",_aTam[03],_aTam[01]	,_aTam[02]	,1,"C","NAOVAZIO()","mv_par06","Somente Vendas"   ,"","","","","Não Vendas"	    ,"","","","","Ambos"			,"","","","","Consignacao","","","","","Venda Consignada","","","",""      ,"",""})
	_aTam  := {03,00,"N"}
