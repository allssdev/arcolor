...
//*******************************************************************
// TODAS AS CUSTOMIZAÇÕES AQUI ENCONTRADAS ESTÃO IDENTIFICADAS POR:
// CUSTOM. ALL - DATA: 99/99/9999 - AUTOR: ANALISTA
// ONDE:
// ----
// CUSTOM. ALL       - IDENTIFICAÇÃO FIXA
// DATA: 99/99/9999 - DATA DO IMPLEMENTO
// AUTOR: ANALISTA  - NOME DO ANALISTA QUE EFETUOU O IMPLEMENTO
//*******************************************************************
...
#DEFINE HMARGEM    030
#DEFINE VMARGEM    030
//CUSTOM. ALL - DATA: 18/04/2023 - AUTOR: DIEGO RODRIGUES
...
#DEFINE MAXITEM    037					// Máximo de produtos para a primeira página
#DEFINE MAXITEMP2  108					// Máximo de produtos para a pagina 2 em diante
#DEFINE MAXITEMP2F 128					// Máximo de produtos para a página 2 em diante quando a página não possui informações complementares
//FIM CUSTOM. ALL - DATA: 18/04/2023 - AUTOR: DIEGO RODRIGUES
...
//CUSTOM. ALL - DATA: 17/01/2013 - AUTOR: ANDERSON C. P. COELHO
...
//CUSTOM. ALL - DATA: 15/08/2018 - AUTOR: ANDERSON C. P. COELHO
	local _nContAux  := 0
//FIM CUSTOM. ALL - DATA: 15/08/2018 - AUTOR: ANDERSON C. P. COELHO
//CUSTOM. ALL - DATA: 29/04/2019 - AUTOR: ANDERSON C. P. COELHO
	local _lRFATR011 := ExistBlock("RFATR011")
//FIM CUSTOM. ALL - DATA: 29/04/2019 - AUTOR: ANDERSON C. P. COELHO

//CUSTOM. ALL - DATA: 11/02/2023 - AUTOR: DIEGO RODRIGUES
	Local _cAlias   := IIF(Type("_cMV_01_NFE")=="C",IIF(SubStr(_cMV_01_NFE,1,1)=="1","SF2","SF1"),Alias())
	Local _cRotina := "DANFEII"

//FIM CUSTOM. ALL - DATA: 11/02/2023 - AUTOR: DIEGO RODRIGUES
...
private oRetNF
//CUSTOM. ALL - DATA: 17/01/2013 - AUTOR: ANDERSON C. P. COELHO
//	Private _cRotina := "DANFEII"
//FIM CUSTOM. ALL - DATA: 17/01/2013 - AUTOR: ANDERSON C. P. COELHO
//CUSTOM. ALL - DATA: 14/05/2015 - AUTOR: ANDERSON C. P. COELHO
//	Private _cAlias   := IIF(Type("_cMV_01_NFE")=="C",IIF(SubStr(_cMV_01_NFE,1,1)=="1","SF2","SF1"),Alias())
//FIM CUSTOM. ALL - DATA: 14/05/2015 - AUTOR: ANDERSON C. P. COELHO
//CUSTOM. ALL - DATA: 17/09/2013 - AUTOR: ADRIANO LEONARDO DE SOUZA
	Private _cNumPed  := ""
	Private _cTipDiv  := ""
	Private _cTransp  := ""
//CUSTOM. ALL - DATA: 11/02/2023 - AUTOR: DIEGO RODRIGUES
If FunName() == "SPEDNFE"
	Private _aNotaImp := {}
	//CUSTOM. ALL - DATA: 08/08/2014 - AUTOR: ADRIANO LEONARDO
	Private aGrvSF3   := {}
	//FIM CUSTOM. ALL - DATA: 08/08/2014 - AUTOR: ADRIANO LEONARDO
EndIf
//FIM CUSTOM. ALL - DATA: 11/02/2023 - AUTOR: DIEGO RODRIGUES
//FIM CUSTOM. ALL - DATA: 17/09/2013 - AUTOR: ADRIANO LEONARDO DE SOUZA
//CUSTOM. ALL - DATA: 27/09/2013 - AUTOR: ADRIANO LEONARDO DE SOUZA
	Private _cCaminho := ""
	Private _cArquivo := ""
//FIM CUSTOM. ALL - DATA: 27/09/2013 - AUTOR: ADRIANO LEONARDO DE SOUZA
//CUSTOM. ALL - DATA: 02/10/2013 - AUTOR: ADRIANO LEONARDO DE SOUZA
	Private _cAnexo	  := ""
	Private _cCarteir := ""
//FIM CUSTOM. ALL - DATA: 02/10/2013 - AUTOR: ADRIANO LEONARDO DE SOUZA
//CUSTOM. ALL - DATA: 14/05/2015 - AUTOR: ANDERSON C. P. COELHO
	_nSeq    := 1
	_aSavPar := {}
...
If nTipo <> 1
	...
	If lJob .or. oSetup:GetProperty(PD_PRINTTYPE) == IMP_PDF
		...
		//INICIO    - CUSTOM. ALL - DATA: 23/06/2014 - AUTOR: ADRIANO LEONARDO DE SOUZA	
			//Defino C:\Danfe\ como pasta default para geração do arquivo (Danfe) na impressão em PDF
			_cBkpPath := oDanfe:cPathPDF
			If ExistDir("c:\danfe\")
				oDanfe:cPathPDF := oSetup:aOptions[PD_VALUETYPE] := "c:\danfe\"
			Else
				_nCriaDir := MakeDir("c:\danfe\")
				
				If _nCriaDir == 0
					oDanfe:cPathPDF := oSetup:aOptions[PD_VALUETYPE] := "c:\danfe\"
				Else
					MsgAlert("Não foi possível criar a pasta C:\Danfe\, logo será utilizada a pasta definida acima!","DANFEII_001")
					oDanfe:cPathPDF := _cBkpPath
				EndIf
			EndIf
		//FIM       - CUSTOM. ALL - DATA: 23/06/2014 - AUTOR: ADRIANO LEONARDO DE SOUZA
		//INICIO - CUSTOM. ALL - DATA: 27/09/2013 - AUTOR: ADRIANO LEONARDO DE SOUZA
			//Armazena o local e nome do arquivo da Danfe para posterior cópia do arquivo para o servidor
			_cCaminho := oDanfe:cPathPDF
			_cArquivo := oDanfe:cFileName		
		//FIM    - CUSTOM. ALL - DATA: 27/09/2013 - AUTOR: ADRIANO LEONARDO DE SOUZA
	...
	//CUSTOM. ALL - DATA: 18/06/2015 - INÍCIO - AUTOR: JÚLIO SOARES
		...
		//endif
			...
			If SF2->(FieldPos("F2_REMPARC"))>0
				...
				Else
					MSGBOX("A DANFe não pode ser impressa porque as parcelas não foram geradas corretamente. INFORME O ADMINISTRADOR DO SISTEMA","DANFEII_001","ALERT")
					Return()
				EndIf
			Else
				MSGBOX("O campo F2_REMPARC não existe no banco de dados. INFORME O ADMINISTRADOR DO SISTEMA","DANFEII_002","STOP")
			EndIf
			*/
			//If lPergunte
				if lJob
			...
			//EndIf
		//FIM CUSTOM. ALL - DATA: 18/06/2015 - AUTOR: JÚLIO SOARES
...
//CUSTOM. ALL - DATA: 09/09/2014 - INÍCIO - AUTOR: JÚLIO SOARES
	// - Condição inserida para impedir que o envio dos boletos e romaneios sejam enviados apenas para as operações de saída.
	// - TIPO DE DOCUMENTO 1=ENTRADA / 2=SAIDA
	If MV_PAR04 == 2
		If lExistNfe
			//If ValType("oDanfe")=="O"
				//oDanfe:Preview()//Visualiza antes de imprimir
			//EndIf
			//INICIO - CUSTOM. ALL - DATA: 27/09/2013 - AUTOR: ADRIANO LEONARDO DE SOUZA
				If File(_cCaminho+StrTran(_cArquivo,".rel",".pdf")) .Or. File(_cCaminho+_cArquivo)
					_cAnexo  := (_cCaminho+StrTran(_cArquivo,".rel",".pdf"))
					CpyT2S(_cAnexo, "\boletos", .F.)
					_cAnexo  := ("\boletos\" + StrTran(_cArquivo,".rel",".pdf"))
				EndIf
			//FIM - CUSTOM. ALL - DATA: 27/09/2013 - AUTOR: ADRIANO LEONARDO DE SOUZA
			//INICIO - CUSTOM. ALL - DATA: 17/09/2013 - AUTOR: ADRIANO LEONARDO DE SOUZA
				//Faço o backup dos parâmetros pois serão sobrescritos na abertura do RFATR011
				_cPar01   := MV_PAR01
				_cPar02   := MV_PAR02
				_cPar03   := MV_PAR03
				//Chamada da rotina de impressão de boletos
				ImpBoletos()
			//FIM - CUSTOM. ALL - DATA: 17/09/2013 - AUTOR: ADRIANO LEONARDO DE SOUZA
		Else
			//INÍCIO - CUSTOM. ALL - DATA: 26/09/2013 - AUTOR: ADRIANO LEONARDO DE SOUZA - PARA IMPRESSÃO DO ROMANEIO DE ENTREGA
				_lImpres  := .F.
				...
				_cPar02   := MV_PAR02
				_cPar03   := MV_PAR03
				For _nContAux := Val(_cPar01) To Val(_cPar02)
					dbSelectArea("SD2")
					SD2->(dbSetOrder(3))
					If SD2->(MsSeek(xFilial("SD2") + _cNumNota + _cPar03))
						If _lRFATR011			//CUSTOM. ALLSS - 29/04/2019 - Anderson Coelho - Trecho alterado para melhoria de perfomance. Conteúdo anterior: ExistBlock("RFATR011")
							_lImpres := .T.
							_lImprime:= .F.
							_lEnvMail:= .F.
							//_cDanfe  := "\boletos\" + _cArquivo
							U_RFATR011(SD2->D2_PEDIDO,SD2->D2_EMISSAO,_lImprime, _lEnvMail)
						EndIf
					EndIf
					_cNumNota := StrZero((Val(_cNumNota)+1),Len(_cPar01))
				Next
				If !_lImpres
					Aviso("DANFE","Nenhuma NF-e a ser impressa nos parametros utilizados.",{"OK"},3)
				EndIf
				RestArea(_aSavSD2)
			//FIM - CUSTOM. ALL - DATA: 26/09/2013 - AUTOR: ADRIANO LEONARDO DE SOUZA - PARA IMPRESSÃO DO ROMANEIO DE ENTREGA
		EndIf
	//Else
		//oDanfe:Preview() //Visualiza antes de imprimir
	EndIf
//FIM CUSTOM. ALL - DATA: 09/09/2014 - AUTOR: JÚLIO SOARES
...
//INICIO CUSTOM. ALL - DATA: 14/05/2015 - AUTOR: ANDERSON C. P. COELHO
...
//CUSTOM. ALL - DATA: 11/02/2023 - AUTOR: DIEGO RODRIGUES
If FunName() == "SPEDNFE"
//CUSTOM. ALL - DATA: 05/04/2013 - AUTOR: ANDERSON C. P. COELHO
	cPerg := Padr("NFSIGW",10)
	Pergunte("NFSIGW",.F.) //Linha adicionada por Adriano Leonardo em 26/06/2014 para correção
	If !ExistBlock("RCFGASX1")
		...
		If (_cAliasSX1)->(MsSeek(cPerg+"01"))
			while !RecLock(_cAliasSX1,.F.) ; enddo
			(_cAliasSX1)->X1_CNT01 := MV_PAR01 := &("(_cAlias)->"+SubStr(_cAlias,2,2)+"_DOC"  )
			(_cAliasSX1)->(MsUnLock())
		EndIf
		If (_cAliasSX1)->(MsSeek(cPerg+"02"))
			while !RecLock(_cAliasSX1,.F.) ; enddo
			(_cAliasSX1)->X1_CNT01 := MV_PAR02 := &("(_cAlias)->"+SubStr(_cAlias,2,2)+"_DOC"  )
			(_cAliasSX1)->(MsUnLock())
		EndIf
		If (_cAliasSX1)->(MsSeek(cPerg+"03"))
			while !RecLock(_cAliasSX1,.F.) ; enddo
			(_cAliasSX1)->X1_CNT01 := MV_PAR03 := &("(_cAlias)->"+SubStr(_cAlias,2,2)+"_SERIE")
			(_cAliasSX1)->(MsUnLock())
		EndIf
		If (_cAliasSX1)->(MsSeek(cPerg+"04"))
			while !RecLock(_cAliasSX1,.F.) ; enddo
			(_cAliasSX1)->X1_CNT01 := MV_PAR04 := IIF(AllTrim(_cAlias)=="SF1",1,2)
			(_cAliasSX1)->(MsUnLock())
		EndIf
	Else
		//Alternativa inserida por Anderson C. P. Coelho em 14.05.2015
		U_RCFGASX1(cPerg,"01",(MV_PAR01 := &("(_cAlias)->"+SubStr(_cAlias,2,2)+"_DOC"  )))
		U_RCFGASX1(cPerg,"02",(MV_PAR02 := &("(_cAlias)->"+SubStr(_cAlias,2,2)+"_DOC"  )))
		U_RCFGASX1(cPerg,"03",(MV_PAR03 := &("(_cAlias)->"+SubStr(_cAlias,2,2)+"_SERIE")))
		U_RCFGASX1(cPerg,"04",(MV_PAR04 := IIF(AllTrim(_cAlias)=="SF1",1,2)             ))
	EndIf
//FIM - CUSTOM. ALL - DATA: 05/04/2013 - AUTOR: ANDERSON C. P. COELHO
EndIf
//FIMCUSTOM. ALL - DATA: 11/02/2023 - AUTOR: DIEGO RODRIGUES
//INICIO - CUSTOM. ALL - DATA: 15/08/2018 - AUTOR: ANDERSON C. P. COELHO
//If Pergunte("NFSIGW",.T.)
//If .T.		//Pergunte("NFSIGW",.T.)
//FIM - CUSTOM. ALL - DATA: 15/08/2018 - AUTOR: ANDERSON C. P. COELHO
...
If !lImpDir .or. MV_PAR04 == 0 /* Caso impressão de DANFE seja realizada via AutoDistMail */
	...
	While !Eof() .And. xFilial("SF3") == (cAliasSF3)->F3_FILIAL .And.;
					...
					If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5") .And. !cCodRetNFE $ RetCodDene()
						...
						Else
							...
							If MsSeek(xFilial("SF2")+aNotas[nX][05]+aNotas[nX][04]+aNotas[nX][06]+aNotas[nX][07]) .And. Alltrim(aXML[nX][8])$"1,3,4,6,7" .Or. ( Alltrim(aXML[nX][8]) $ "2,5"  .And. !Empty(cAutoriza) )
									//CUSTOM. ALL - DATA: 17/01/2013 - AUTOR: ANDERSON C. P. COELHO
										If SF2->(FieldPos("F2_ENDEXP"))>0
											//CUSTOM. ALL - DATA: 11/02/2023 - AUTOR: DIEGO RODRIGUES
											If Empty(SF2->F2_ENDEXP) .And. SF2->F2_TIPO=="N" .and. FunName() == "SPEDNFE"
												MsgAlert("A nota fiscal " + SF2->F2_DOC + ", série " + SF2->F2_SERIE + " não foi endereçada na expedição. Impressão da DANFE não autorizada!",_cRotina+"_001")
												Loop
											EndIf
											//FIM CUSTOM. ALL - DATA: 11/02/2023 - AUTOR: DIEGO RODRIGUES
										EndIf
								...
								MsUnlock()
									//CUSTOM. ALL - DATA: 11/06/2013 - AUTOR: Júlio Soares
										_cNota := (xFilial("SF2") + aNotas[nX][04] + aNotas[nX][05])
										if SF2->(Alltrim(F2_ESPECIE)) $ "SPED"
											dbSelectArea("SE1")
											SE1->(dbSetOrder(1))
											if SE1->(MsSeek(SE1->(_cNota),.T.,.F.))
												while SE1->((xFilial("SE1")) + E1_PREFIXO + E1_NUM) == _cNota .AND. E1_FLUXO <> "S"
												...
												MsgAlert("O título " + SF2->F2_SERIE + " - " + SF2->F2_DOC + " NÃO foi atualizado no Financeiro, informe o Administrador do sistema ",_cRotina+"_003")
											endif
										endif
									//FIM - CUSTOM. ALL - DATA: 11/06/2013 - AUTOR: Júlio Soares  // Customização replicada na linha 544
						...
						IF SF1->(!EOF()) .And. SF2->(FieldPos("F2_FILDEST"))> 0 .And. SF2->(FieldPos("F2_FORDES"))> 0 .And.SF2->(FieldPos("F2_LOJADES"))> 0 .And.SF2->(FieldPos("F2_FORMDES"))> 0 .And. !EMPTY (SF2->F2_FORDES)
							...
							If SFT->(MsSeek(cChaveSFT))
								Do While cChaveSFT == SFT->FT_FILIAL+"E"+SFT->FT_SERIE+SFT->FT_NFISCAL+SFT->FT_CLIEFOR+SFT->FT_LOJA .And. !SFT->(Eof())
									RecLock("SFT")
									SFT->FT_CHVNFE := SubStr(NfeIdSPED(aXML[nX][2],"Id"),4)
									//INICIO - CUSTOM. ALL - DATA: 19/07/2018 - AUTOR: ANDERSON C. P. COELHO - Inserido pois estava na versão padrão anterior e, analisando a lógica, entendemos que seria importante neste fonte. Avaliar, posteriormente, a necessidade de eliminar este trecho.
											SFT->FT_CODNFE := cAutoriza
									//FIM - CUSTOM. ALL - DATA: 19/07/2018 - AUTOR: ANDERSON C. P. COELHO - Inserido pois estava na versão padrão anterior e, analisando a lógica, entendemos que seria importante neste fonte. Avaliar, posteriormente, a necessidade de eliminar este trecho.
									MsUnLock()
									//Array criado para gravar o SF3 no final, pois a tabela SF3 pode estah em processamento quando se trata de DBF ou AS/400.
									If aScan(aGrvSF3,{|aX|aX[1]+aX[2]+aX[3]+aX[4]+aX[5]==(cAliasSFT)->(FT_SERIE+FT_NFISCAL+FT_CLIEFOR+FT_LOJA+FT_IDENTF3)})==0
										aAdd(aGrvSF3, {(cAliasSFT)->FT_SERIE,(cAliasSFT)->FT_NFISCAL,(cAliasSFT)->FT_CLIEFOR,(cAliasSFT)->FT_LOJA,(cAliasSFT)->FT_IDENTF3,(cAliasSFT)->FT_CHVNFE,cAutoriza,cCodRetSF3,cMsgSF3})
		...
		EndIf
			//INICIO: CUSTOM. ALL - DATA: 17/09/2013 - AUTOR: ADRIANO LEONARDO DE SOUZA
				If Ascan(_aNotaImp, xFilial("SD2") + SF2->F2_DOC + SF2->F2_SERIE + SF2->F2_CLIENTE + SF2->F2_LOJA) <= 0
					AAdd(_aNotaImp,xFilial("SD2") + SF2->F2_DOC + SF2->F2_SERIE + SF2->F2_CLIENTE + SF2->F2_LOJA) 
				EndIf
			//FIM: CUSTOM. ALL - DATA: 17/09/2013 - AUTOR: ADRIANO LEONARDO DE SOUZA
	EndDo
...
Local cEsp		    := ""
Local cLogoD	    := ""
//CUSTOM. ALL - DATA: 05/01/2022 - AUTOR: RODRIGO TELECIO
//local cLogoTotvs 	:= "Powered_by_TOTVS.bmp"
local cLogoTotvs 	:= ""
//FIM CUSTOM. ALL - DATA: 05/01/2022 - AUTOR: RODRIGO TELECIO
...
//CUSTOM. ALL - DATA: 25/07/2018 - AUTOR: ANDERSON C. P. COELHO - INSERIDO TRATAMENTO DE IMPRESSÃO DAS DUAS UNIDADES DE MEDIDA, APENAS PARA O ESTADO DO MATO GROSSO. DE RESTO, SAIRÁ APENAS COM UMA UNIDADE DE MEDIDA
	Local lUf_MT	:= ( SuperGetMv("MV_ESTADO") $ "MT" )
//FIM CUSTOM. ALL - DATA: 25/07/2018 - AUTOR: ANDERSON C. P. COELHO - INSERIDO TRATAMENTO DE IMPRESSÃO DAS DUAS UNIDADES DE MEDIDA, APENAS PARA O ESTADO DO MATO GROSSO. DE RESTO, SAIRÁ APENAS COM UMA UNIDADE DE MEDIDA
...
For nZ := 1 To nLenDet
	...
	//CUSTOM. ALL - DATA: 16/01/2023 - AUTOR: DIEGO RODRIGUES
	dbSelectArea("SD2")
	SD2->(dbSetOrder(3))
	If SD2->(MsSeek(xFilial("SD2") + SD2->D2_DOC + SD2->D2_SERIE))
		...
		Else
			...
			aadd(aItens,{;
			SubStr(oDet[nX]:_Prod:_cProd:TEXT,1,nMaxCod),;
			SubStr(NoChar(oDet[nX]:_Prod:_xProd:TEXT,lConverte)+If(SD2->D2_TIPO == 'D'," LI:" +Substr(oDet[nX]:_Prod:_Rastro:_nLote:TEXT,1,6)+" LF:" + Alltrim(U_RPCPG002(Substr(oDet[nX]:_Prod:_Rastro:_nLote:TEXT,1,6)))," Lote:" +Substr(oDet[nX]:_Prod:_Rastro:_nLote:TEXT,1,6)),1,nMaxDes),;
			IIF(ValAtrib("oDet[nPrivate]:_Prod:_NCM")=="U","",oDet[nX]:_Prod:_NCM:TEXT),;
			cSitTrib,;
			oDet[nX]:_Prod:_CFOP:TEXT,;
			oDet[nX]:_Prod:_uCom:TEXT,;
		...
		// Tratamento somente para o estado de MG, para totalizar por CFOP conforme RICMS-MG
			...
			cAuxItem := AllTrim(SubStr(oDet[nX]:_Prod:_cProd:TEXT,nMaxCod+1))
			cAux     := AllTrim(SubStr(NoChar(oDet[nX]:_Prod:_xProd:TEXT,lConverte)+If(SD2->D2_TIPO == 'D'," LI:" +Substr(oDet[nX]:_Prod:_Rastro:_nLote:TEXT,1,6)+" LF:" + Alltrim(U_RPCPG002(Substr(oDet[nX]:_Prod:_Rastro:_nLote:TEXT,1,6)))," Lote:" +Substr(oDet[nX]:_Prod:_Rastro:_nLote:TEXT,1,6)),(nMaxDes+1)))
			aAux[1]  := SubStr(aAux[1], PosQuebrVal(aAux[1]) + 1)
			aAux[2]  := SubStr(aAux[2], PosQuebrVal(aAux[2]) + 1)
			aAux[3]  := SubStr(aAux[3], PosQuebrVal(aAux[3]) + 1)
			aAux[4]  := SubStr(aAux[4], PosQuebrVal(aAux[4]) + 1)
		...
		EndIf
	EndIF
	//FIM CUSTOM. ALL - DATA: 16/01/2023 - AUTOR: DIEGO RODRIGUES
	
	//CUSTOM. ALL - DATA: 31/03/2023 - AUTOR: DIEGO RODRIGUES
	If(MV_PAR04==1) 
		dbSelectArea("SD1")
		SD1->(dbSetOrder(1)) //D1_FILIAL+D1_DOC+D1_SERIE+D1_FORNECE+D1_LOJA+D1_COD+D1_ITEM 
		If SD1->(MsSeek(xFilial("SD1") + SD1->D1_DOC + SD1->D1_SERIE + SD1->D1_FORNECE + SD1->D1_LOJA ))
		...
		EndIf
	EndIF
	//FIM DA CUSTOM ALLSS 31/03/2023
...
//CUSTOM. ALL - DATA: 17/01/2013 - AUTOR: ANDERSON C. P. COELHO
	If MV_PAR04 == 2 .AND. SF2->(FieldPos("F2_ENDEXP"))>0 .AND. Type("SF2->F2_ENDEXP")=="C" .AND. !Empty(SF2->F2_ENDEXP)
		cAux := StripTags("###END_"+AllTrim(SF2->F2_ENDEXP)+"_###", .T.)
		While !Empty(cAux)
...
//CUSTOM. ALL - DATA: 05/07/2013 - AUTOR: ADRIANO LEONARDO DE SOUZA - CONTATO DA TRANSPORTADORA
	If MV_PAR04 == 2 .AND. SF2->(FieldPos("F2_ENDEXP"))>0 .AND. Type("SF2->F2_ENDEXP")=="C" .AND. !Empty(SF2->F2_ENDEXP)
		_cCgcTrans := ""
		If Type("oTransp:_Transporta:_CNPJ:TEXT")<>"U"
			_cCgcTrans := Replace(Replace(Replace(aTransp[06],'.',''),'/',''),'-','')
		ElseIf Type("oTransp:_Transporta:_CPF:TEXT")<>"U"
			_cCgcTrans := Replace(Replace(Replace(aTransp[06],'.',''),'/',''),'-','')
		EndIf
		_cDddTrans := Posicione("SA4",3, xFilial("SA4") + _cCgcTrans, "A4_DDD")
		_cTelTrans := Posicione("SA4",3, xFilial("SA4") + _cCgcTrans, "A4_TEL")
	    //CUSTOM. ALL - DATA: 14/11/2013 - AUTOR: ADRIANO LEONARDO DE SOUZA
			_cDddTran2 := Posicione("SA4",3, xFilial("SA4") + _cCgcTrans, "A4_DDD2")
			_cTelTran2 := Posicione("SA4",3, xFilial("SA4") + _cCgcTrans, "A4_TEL2")
		//	cAux2 := StripTags("###Contato da Transportadora: " + AllTrim("(" + _cDddTrans + ")" + " " + _cTelTrans) + "_###", .T.) //Linha comentada por Adriano Leonardo em 14/11/2013
			cAux2 := StripTags("###Contato da Transportadora: " + AllTrim("(" + _cDddTrans + ")" + " " + _cTelTrans) + IIF(!Empty(_cTelTran2), " / " + AllTrim("(" + _cDddTran2 + ")" + " " + _cTelTran2),"") + "_###", .T.) //Linha adicionada por Adriano Leonardo em 14/11/2013
	    //FIM CUSTOM. ALL - DATA: 14/11/2013 - AUTOR: ADRIANO LEONARDO DE SOUZA
		While !Empty(cAux2)
		...
		EndDo
	EndIf
//FIM - CUSTOM. ALL - DATA: 05/07/2013 - AUTOR: ADRIANO LEONARDO DE SOUZA - CONTATO DA TRANSPORTADORA
//CUSTOM. ALL - DATA:17/03/2014 - AUTOR: Júlio Soares - Carteira e formas de pagamento
	If MV_PAR04 == 2 .AND. SF2->(FieldPos("F2_ENDEXP"))>0 .AND. Type("SF2->F2_ENDEXP")=="C" .AND. !Empty(SF2->F2_ENDEXP)
		cAux3 := StripTags("###_" + Alltrim (Posicione("SA1",1, xFilial("SA1")+SC5->(C5_CLIENTE)+SC5->(C5_LOJACLI),"A1_CDCART")) + "_" +Alltrim (SC5->C5_DESCOND) + "_###", .T.)
		While !Empty(cAux3)
			aadd(aMensagem,SubStr(cAux3,1,IIf(EspacoAt(cAux3, MAXMENLIN) > 1, EspacoAt(cAux3, MAXMENLIN) - 1, MAXMENLIN)))
			cAux3 := SubStr(cAux3,IIf(EspacoAt(cAux3, MAXMENLIN) > 1, EspacoAt(cAux3, MAXMENLIN), MAXMENLIN) + 1)
		EndDo
	EndIf
//FIM - CUSTOM. ALL - DATA:17/03/2014 - AUTOR: Júlio Soares - Carteira e formas de pagamento
//CUSTOM. ALL - DATA:17/04/2014 - AUTOR: Adriano Leonardo - Observações da condição de pagamento
	//Certifico que o campo necessário existe
	dbSelectArea("SE4")
	If SE4->(FieldPos("E4_OBSNOTA"))<>0
		cAux4 := StripTags("###_" + Alltrim (Posicione("SE4",1, xFilial("SE4")+SF2->(SF2->F2_COND),"E4_OBSNOTA")) + "_###", .T.)
		While !Empty(cAux4) .And. cAux4 <> ("###__###")
			aadd(aMensagem,SubStr(cAux4,1,IIf(EspacoAt(cAux4, MAXMENLIN) > 1, EspacoAt(cAux4, MAXMENLIN) - 1, MAXMENLIN)))
			cAux4 := SubStr(cAux4,IIf(EspacoAt(cAux4, MAXMENLIN) > 1, EspacoAt(cAux4, MAXMENLIN), MAXMENLIN) + 1)
		EndDo
	EndIf
//FIM - CUSTOM. ALL - DATA:17/04/2014 - AUTOR: Adriano Leonardo - Observações da condição de pagamento
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Programa  ³IMPBOLETOS³ Autor ³ Adriano Leonardo      ³ Data ³26/08/2013³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³Rotina responsável pela impressão e envio do boleto do Banco³±±
±±³          ³do Brasil na sequência da Danfe.                            ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³   P11    ³ Programa principal -> Uso específico Arcolor               ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß*/
//CUSTOM. ALL - DATA: 26/08/2013 - AUTOR: ADRIANO LEONARDO DE SOUZA -- IMPRESSÃO E ENVIO DE BOLETO AUTOMÁTICO
	static function ImpBoletos()
		Local _nCont
		Local nCont2
		Local _aSavSD2 := SD2->(GetArea())
		Local _aSavSC5 := SC5->(GetArea())
		Local _aSavSA1 := SA1->(GetArea())
		_lImpBol       := AllTrim(__cUserId) == "000000" .Or. SuperGetMv("MV_AUTOBOL",,.F. ) //Ativa a rotina de impressão do boleto junto com a DANFE, verifique também o parâmetro MV_AUTOBOL
		If (ExistBlock("RFINR005") .Or. ExistBlock("RFINR017") .Or. ExistBlock("RFINR018")).And. _lImpBol //Chamada da rotina de impressão do boleto
			for _nCont := 1 to Len(_aNotaImp)
			//For _nContNF := 1 To Len(aGrvSF3)
				dbSelectArea("SD2")
				SD2->(dbSetOrder(3))
				SD2->(dbGoTop())
				If SD2->(MsSeek(_aNotaImp[_nCont],.T.,.F.))
				//If MsSeek(xFilial("SF2")+aGrvSF3[_nContNF,2]+aGrvSF3[_nContNF,1]+aGrvSF3[_nContNF,3]+aGrvSF3[_nContNF,4])
				...
				If SC5->(MsSeek(xFilial("SC5")+_cNumPed,.T.,.F.))
					...
					EndIf
				EndIf    
				//CUSTOM. ALL - DATA: 08/08/2014 - AUTOR: ADRIANO LEONARDO DE SOUZA -- IMPRESSÃO E ENVIO DE BOLETO AUTOMÁTICO
					If _cPar01 <> _cPar02
						If !Empty(_cAnexo) .And. ";" $ _cAnexo
							_aAnexo := StrTokArr(_cAnexo,";")
							For nCont2 := 1 To Len(_aAnexo)
								If File(_aAnexo[nCont2])    //Verifico a existência do arquivo
									fErase(_aAnexo[nCont2]) //Apago o arquivo temporário
								EndIf
							Next
						Else
							If File(_cAnexo) .And. !("BOL_" $ _cAnexo) .And. !("romaneio_" $ _cAnexo)	//Verifico a existência do arquivo
								fErase(_cAnexo)         				//Apago o arquivo temporário
							EndIf
						EndIf
						_cAnexo := ""
					EndIf
				//CUSTOM. ALL - DATA: 08/08/2014 - AUTOR: ADRIANO LEONARDO DE SOUZA -- IMPRESSÃO E ENVIO DE BOLETO AUTOMÁTICO
					If _cTipDiv == "4"
						//If AllTrim(_cCarteir) <> "17" .And. ExistBlock("RFINR005") // Linha comentada por Júlio Soares em 02/05/2016
						If AllTrim(_cCarteir) <> "17" .And. AllTrim(_cCarteir) <> "109" .And. AllTrim(_cCarteir) <> "112" .And. ExistBlock("RFINR005")
							U_RFINR005(SD2->D2_SERIE,SD2->D2_DOC,.T.,.T.,.F.,.F.,,,,,_cAnexo) //Imprime boleto carteira 18 e envia boleto e Danfe por e-mail
						ElseIf AllTrim(_cCarteir) == "17" .And. ExistBlock("RFINR017")
							U_RFINR017(SD2->D2_SERIE,SD2->D2_DOC,.T.,.T.,.F.,.F.,,,,,_cAnexo) //Imprime boleto carteira 17 e envia boleto e Danfe por e-mail
						ElseIf AllTrim(_cCarteir) == "109" .And. ExistBlock("RFINR018")
							U_RFINR018(SD2->D2_SERIE,SD2->D2_DOC,.T.,.T.,.F.,.F.,,,,,_cAnexo)
						EndIf
					ElseIf _cTipDiv <> "0" .And. _cTipDiv <> "4"
						_cHrDoc := Posicione("SF2",1,xFilial("SF2")+SD2->(D2_DOC+D2_SERIE+D2_CLIENTE+D2_LOJA),"F2_HORA")
						If !(_cTransp $ SuperGetMv("MV_AIBTRAN" ,,"001260" ))
							U_RFATR011(_cNumPed,SD2->D2_EMISSAO,.F.,.T.,_cAnexo,_cHrDoc)  //Imprime e envia Danfe, romaneio e boletos ao cliente
						Else
							U_RFATR011(_cNumPed,SD2->D2_EMISSAO,.T.,.T.,_cAnexo,_cHrDoc)  //Envia romaneio e boleto e Danfe ao cliente
						EndIf
					EndIf
				//FIM CUSTOM. ALL - DATA: 08/08/2014 - AUTOR: ADRIANO LEONARDO DE SOUZA -- IMPRESSÃO E ENVIO DE BOLETO AUTOMÁTICO
			next
		EndIf
		RestArea(_aSavSA1)
		RestArea(_aSavSC5)
		RestArea(_aSavSD2)
	return
//FIM CUSTOM. ALL - DATA: 26/08/2013 - AUTOR: ADRIANO LEONARDO DE SOUZA
