...
/*/{Protheus.doc} RFATR053
Rotina automática para envio de e-mail da nova meta de vendas para representantes comerciais
RESUMO POR SUPERVISOR
@author Rodrigo Telecio (rodrigo.telecio@allss.com.br) - ALLSS Soluções em Sistemas
@since 02/08/2022
@version P12.1.33
@type Function
@obs Sem observações
@see https://allss.com.br/
@history 02/08/2022, Rodrigo Telecio (rodrigo.telecio@allss.com.br), Versão inicial da rotina.
@history 22/08/2022, Rodrigo Telecio (rodrigo.telecio@allss.com.br), Aplicação no ambiente de produção.
@history 24/08/2022, Rodrigo Telecio (rodrigo.telecio@allss.com.br), Ajuste com adição de percentuais no resumo de cada supervisor e no resumo da meta global.
@history 31/08/2022, Rodrigo Telecio (rodrigo.telecio@allss.com.br), Ajuste com remoção de símbolo monetário nos campos percentuais.
@history 14/09/2022, Rodrigo Telecio (rodrigo.telecio@allss.com.br), Adição de data da posição do e-mail e troca do termo "SALDO" para "SALDO".
@history 03/11/2022, Diego Rodrigues (diego.rodrigues@allss.com.br), Alteração da ordem de calculo do saldo "_nReal - _nMeta" e troca do termo "FALTANTE" para "SALDO".
@history 23/02/2023, Diego Rodrigues (diego.rodrigues@allss.com.br), Alteração da titulo do e-mail removendo a palavra campanha.
@history 02/03/2023, Diego Rodrigues (diego.rodrigues@allss.com.br), Alteração da titulo do e-mail inserindo periodo como nome e removido o bloco de meta global
/*/
user function RFATR053(_cTipo)
local   nAntes  				:= 2
local   dDeData 				:= StoD('')
local   dAtData 				:= StoD('')
...
private cRotina 				:= ""
private oProcessa
private cArquivo 				:= "RFATR053"
private cTitulo  				:= "Acompanhamento das Metas de Vendas - Resumo por Supervisor"
private cText					:= "Acompanhamento das Metas de Vendas - Resumo por Supervisor"
//private lBlind 				:= .T. //temporário, reativar
...
if lBlind
	...
	cRotina 				:= "RFATR053 - " + AllTrim(FunName()) + " (Automático)"
	cUsuario				:= "Administrador"
	cArqLogs				:= AllTrim(SuperGetMv('MV_XARQLOG' ,.F.,'\04. Logs\'))
	AADD(aArqsLog,"INICIO - " + AllTrim(cRotina) + " - Acompanhamento das Metas de Vendas - Resumo por Supervisor(a)")
	AADD(aArqsLog,"--------------------------------------------------------------------------------")
	AADD(aArqsLog,"Data/Hora do inicio de processamento: "      + AllTrim(DtoC(Date()))     + " - "         + AllTrim(Time()))
	AADD(aArqsLog,"*************************** EXECUÇÃO AUTOMÁTICA ********************************")
	AADD(aArqsLog,"Usuário responsável pelo processamento: "    + AllTrim(RetCodUsr())      + " - "         + UsrFullName(RetCodUsr()))
	AADD(aArqsLog,"Informações do ambiente: Protheus "          + AllTrim(GetRPORelease())  + " - Build "   + AllTrim(GetBuild(.F.)))
	AADD(aArqsLog,"--------------------------------------------------------------------------------")
    //****************************************************************************************
    ...
    if AllTrim(DtoS(dDeData)) != '' .OR. AllTrim(DtoS(dAtData)) != ''
        AADD(aArqsLog,"Campanha ativa e selecionada para composição de metas e valores: " + AllTrim(cMeta + '-' + cTitMeta))
        AADD(aArqsLog,"--------------------------------------------------------------------------------")
	    ProcMail1(_cTipo,dDeData,dAtData,cMeta,cTitMeta,.F.)
    else
        AADD(aArqsLog,"Não foi encontrada uma campanha ativa para composição de metas e valores.")
        AADD(aArqsLog,"--------------------------------------------------------------------------------")
    endif
else
	...
	if cEmpExec + cFilExec <> FWCodEmp() + FWCodFil()
    	Aviso('TOTVS','Empresa/filial nao autorizada para executar tal rotina.',{'&OK'},3,'Cancelamento de operação')
        ...
        if AllTrim(DtoS(dDeData)) != '' .OR. AllTrim(DtoS(dAtData)) != ''
            ...
            @ 002,002 to 090,190
            @ 010,003 Say '  Acompanhamento das Metas de Vendas - Resumo por Supervisor(a)                   '
            @ 020,003 Say '  Campanha ativa e disponível: ' + AllTrim(cMeta) + '-' + AllTrim(cTitMeta) + '.  '
...
±±ºPrograma  ³ ProcMail1  ºAutor  ³Rodrigo Telecio    º Data ³  11/07/2022º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ Processa resultados e envio e-mail                         º±±
...
//****************************************************************************************
//Query para levantamento do valor GLOBAL realizado, dentro do período da meta determinada
//****************************************************************************************
...
(cAliasTotal)->(dbCloseArea())
//*******************************************************************************************************
//Query para levantamento do valor INDIVIDUAL previsto e realizado, dentro do período da meta determinada
//*******************************************************************************************************
...
BeginSql Alias cAliasMeta
    ...
    SELECT
        ...
        ROUND(SUM(ISNULL(VENDAS.CT_QUANT,0)),2)										        AS CT_QUANT,
        ROUND(SUM(ISNULL(VENDAS.CT_VALOR,0)),2)										        AS CT_VALOR,
    ...
    GROUP BY
        ...
        VENDAS.A3_GEREN, VENDAS.GER_EMAIL, VENDAS.A3_SUPER, VENDAS.SUP_NOME, VENDAS.SUP_EMAIL, VENDAS.CT_DATA
    ORDER BY
        VENDAS.A3_GEREN, VENDAS.A3_SUPER, SUO.UO_DTINI
EndSql
MemoWrite("\2.MemoWrite\" + cRotina + "_QRY_003.TXT",GetLastQuery()[02])
dbSelectArea(cAliasMeta)
ProcRegua((cAliasMeta)->(RecCount()))
_cMail          := SuperGetMV("MV_XFRO053"  ,.F.,'rodrigo.telecio@allss.com.br')
_cCC 	        := SuperGetMV("MV_XCC053"   ,.F.,'rodrigo.telecio@allss.com.br')
_cBCC           := SuperGetMV("MV_XBCC053"  ,.F.,'rodrigo.telecio@allss.com.br')
//_cAssunto       := "[ARCOLOR] Acompanhamento de vendas - Posição em " + DtoC(dDataBase) + " - Período da Campanha de " + DtoC(_dDeData) + " a " + DtoC(_dAtData) + " - Resumo Geral por Supervisor(a)"
//_cHtml          := "<H2>Acompanhamento de vendas - Posição em " + DtoC(dDataBase) + " - Período da Campanha de " + DtoC(_dDeData) + " a " + DtoC(_dAtData) + " - Resumo Geral por Supervisor(a)" + CLRF
//_cAssunto       := "[ARCOLOR] Acompanhamento de vendas - Posição em " + DtoC(dDataBase) + " - Período de " + DtoC(_dDeData) + " a " + DtoC(_dAtData) + " - Resumo Geral por Supervisor(a)"
//_cHtml          := "<H2>Acompanhamento de vendas - Posição em " + DtoC(dDataBase) + " - Período de " + DtoC(_dDeData) + " a " + DtoC(_dAtData) + " - Resumo Geral por Supervisor(a)" + CLRF
_cAssunto       := "[Arcolor] Acompanhamento de vendas - Posição em " + AllTrim((cAliasMeta)->UO_DESC)   + " - Referente ao mês " + DtoC(_dDeData) + " a " + DtoC(_dAtData) + " - Resumo Geral por Supervisor(a)"
//<img width='500' height='200' src='cid:ID_" + _cLogo + "'>
_cHtml          := "<H2>Acompanhamento de vendas - Posição em " + AllTrim((cAliasMeta)->UO_DESC)   + " - Referente ao mês " + DtoC(_dDeData) + " a " + DtoC(_dAtData) + " - Resumo Geral por Supervisor(a)" + CLRF
while !(cAliasMeta)->(EOF()) .AND. !lEnd  
    ...
    _cHtml          += "        <tr> "                                                              + CLRF
    _cHtml          += "            <th colspan='4'> SUPERVISOR(A) - " + AllTrim((cAliasMeta)->A3_SUPNOME) + "</th> "   + CLRF
    _cHtml          += "        </tr> "                                                             + CLRF
    _cHtml          += "    </thead> "                                                              + CLRF
    _cHtml          += " 	<thead bgcolor='#808080'> "                                             + CLRF
    ...
    while !(cAliasMeta)->(EOF()) .AND. !lEnd .AND. _cSupervisor == (cAliasMeta)->A3_SUPER
        ...
        (cAliasMeta)->(dbSkip())  
    enddo
    /*Comentado total representante devido a mudança de layout e informação redundante
    //********************************************************************************
    //Valores totais por supervisor(a)
    //********************************************************************************
    _cHtml          += " 		<tr> "                                                                                                                                                  + CLRF
    _cHtml          += " 			<th valign='top' align='center' border='1' width='150'>"    + "TOTAL META SUPERVISOR(A)"                                            + "</th> "      + CLRF
    ...
    */    
	...
	dbSelectArea(cAliasMeta)
enddo
//Comentado a meta global a solicitação da Carla
/*
//********************************************************************************
...
if lRetMail
    ...
    AADD(aArqsLog,'Destinatário: ' 	+ AllTrim(_cMail))
    AADD(aArqsLog,'Cópia: ' 		+ AllTrim(_cCC))
    AADD(aArqsLog,'Cópia oculta: ' 	+ AllTrim(_cBCC))
    AADD(aArqsLog,"--------------------------------------------------------------------------------")
else
...
AADD(aArqsLog,"Data/Hora de término de processamento: " + AllTrim(DtoC(Date())) + " - " + AllTrim(Time()))
AADD(aArqsLog,"TÉRMINO - " + AllTrim(cRotina))
AADD(aArqsLog,"--------------------------------------------------------------------------------")
if Len(aArqsLog) > 0
...
if !lBlind
	...
	Aviso('TOTVS','Término do processamento. Verifique o arquivo de log gerado pelo processamento e confira os resultados.',{'&OK'},3,'Término de processamento')
	Close(oProcessa)
else
...
/*/{Protheus.doc} RFATR51PV
    Função para buscar quantidade de Pedidos de Venda do representante
    ...
    @return _nQtdPv
/*/
Static Function RFATR51PV(_dDataDe, _dDataAte, _cSuper)
Local _nQtdPv := 0
