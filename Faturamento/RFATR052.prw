...
@history 24/08/2022, Rodrigo Telecio (rodrigo.telecio@allss.com.br), Ajuste com adição de percentuais no resumo da meta global.
@history 31/08/2022, Rodrigo Telecio (rodrigo.telecio@allss.com.br), Ajuste com adição de percentuais no resumo da meta do representante.
...
@history 16/11/2022, Diego Rodrigues (diego.rodrigues@allss.com.br), Alteração da ordem de calculo do saldo "_nReal - _nMeta" para supervisores e geral
...
if lBlind
	...
	cRotina 				:= "RFATR052 - " + AllTrim(FunName()) + " (Automático)"
...
while !(cAliasMeta)->(EOF()) .AND. !lEnd
    ...
    //_cAssunto       := "[ARCOLOR] Acompanhamento de vendas - Posição em " + DtoC(dDataBase) +  " - Período da Campanha de " + DtoC(_dDeData) + " a " + DtoC(_dAtData) + " - Resumo do Supervisor(a) - " + AllTrim((cAliasMeta)->A3_SUPNOME)
    //_cHtml          := "<H2>Acompanhamento de vendas - Posição em " + DtoC(dDataBase) + " - Período da Campanha de " + DtoC(_dDeData) + " a " + DtoC(_dAtData) + " - Resumo do Supervisor(a) - " + AllTrim((cAliasMeta)->A3_SUPNOME) + CLRF
